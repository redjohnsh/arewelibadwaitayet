	...
	for (const [id, lang] of Object.entries(appLanguages)) {
		...
		if (appstreamApp) {
			result[id] = {
				...
				lang: lang,
				categories: appstreamApp.categories,
...
export async function getEnrichedAppsData(
	appLanguages: Record<string, string>
): Promise<Record<string, AppWithMetadata>> {
	...
	console.log(
		`Included ${includedCount} apps out of ${totalConfigured} configured apps (${totalConfigured - includedCount} apps not found in AppStream)`
	);

	return enrichedApps;
}

export async function fetchAndParseAppStream(
	targetAppIds: Set<string>
): Promise<Map<string, AppstreamApp>> {
		...
		nodeStream
			...
			.on('end', () => {
				try {
					...
					console.log(
						`Found ${foundApps.size} apps in AppStream data out of ${targetAppIds.size} configured apps`
					);
					resolve(foundApps);
				} catch (error) {
					reject(
						new Error(
							`XML parsing error: ${error instanceof Error ? error.message : String(error)}`
						)
					);
				}
			})
...
function extractAppsFromAppStream(
	xmlString: string,
	targetAppIds: Set<string>
): Map<string, AppstreamApp> {
	...
	try {
		...
		let components = result?.components?.component || result?.component || [];
		...
		for (const component of componentsArray) {
			try {
				...
				if (component.categories) {
					if (Array.isArray(component.categories.category)) {
						categories = component.categories.category
							.map((cat: any) => (typeof cat === 'string' ? cat : cat['#text'] || cat))
							.filter(Boolean);
					} else if (component.categories.category) {
						// Single category
						const cat = component.categories.category;
						categories = [typeof cat === 'string' ? cat : cat['#text'] || cat].filter(Boolean);
					}
				}
				...
				if (name && summary) {
					...
					console.warn(
						`App ${appId} found but missing name or summary (name: "${name}", summary: "${summary}")`
					);
				}
			} catch (error) {
	...
	if (foundApps.size < 200) {
		console.warn(
			`Only found ${foundApps.size} apps with fast-xml-parser, falling back to regex parsing...`
		);
		return extractAppsWithRegex(xmlString, targetAppIds);
	}
...
function extractAppsWithRegex(
	...
	while ((componentMatch = componentRegex.exec(xmlString)) !== null) {
		...
		try {
			...
			const categories = extractCategories(componentXml);
			...
			if (name && summary) {
				...
				console.warn(
					`App ${targetId} found but missing name or summary (name: "${name}", summary: "${summary}")`
				);
			}
		} catch (error) {
...
// Extract categories from XML using regex
function extractCategories(xml: string): string[] {
	...
	// Extract individual category elements
	const categoryRegex = /<category>([^<]+)<\/category>/gi;
	let categoryMatch;
	while ((categoryMatch = categoryRegex.exec(categoriesXml)) !== null) {
...
function extractElementText(xml: string, elementName: string): string | null {
	...
	const multilineRegex = new RegExp(
		`<${elementName}[^>]*>\\s*([\\s\\S]*?)\\s*</${elementName}>`,
		'i'
	);
